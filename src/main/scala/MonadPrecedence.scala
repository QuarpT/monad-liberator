...
trait DeepFlatMap[A[_], B] {
  def apply[C](a: A[B])(f: B => C): A[C]
}

object DeepFlatMap {
  def apply[F[_], G[_], A, B](a: F[G[A]])(f: A => B)(implicit mm: DeepFlatMap[Lambda[X => F[G[X]]], A]): F[G[B]] = mm(a)(f)
  def apply2[F1[_], F2[_], F3[_], B, C](a: F1[F2[F3[B]]])(f: B => C)(implicit mm: DeepFlatMap[Lambda[X => F1[F2[F3[X]]]], B]): F1[F2[F3[C]]] = mm(a)(f)
  implicit class DeepMap1[F1[_], A](functor: F1[A])(implicit mm: DeepFlatMap[Lambda[X => F1[X]], A]) {
    def mp = this
    def mp1 = this
  ...
  implicit class DeepMap2[F1[_], F2[_], A](functor: F1[F2[A]])(implicit mm: DeepFlatMap[Lambda[X => F1[F2[X]]], A]) {
    def mp = this
    def mp2 = this
  ...
  implicit class DeepMap3[F1[_], F2[_], F3[_], A](functor: F1[F2[F3[A]]])(implicit mm: DeepFlatMap[Lambda[X => F1[F2[F3[X]]]], A]) {
    def mp = this
    def mp3 = this
...
trait HighPriorityImplicits extends MediumPriorityImplicits {
  ...
  implicit def deepMapRule2[F[_] : Functor, G[_] : Functor, B] = new DeepFlatMap[Lambda[X => F[G[X]]], B] {
    override def apply[C](a: F[G[B]])(f: B => C): F[G[C]] = {
      Functor[F].map(a)(_.map(f))
    }
  }
  implicit def deepMapRule3[F1[_] : Functor, F2[_] : Functor, F3[_] : Functor, B] = new DeepFlatMap[Lambda[X => F1[F2[F3[X]]]], B] {
